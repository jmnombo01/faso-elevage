import { PrismaClient, Espece } from '@prisma/client';
...
const usersData = [
  ...
  { phone: '+22670000099', name: 'Admin Faso', ville: 'Ouagadougou', quartier: 'Ouaga 2000', role: 'ADMIN' as const },
];
...
async function main() {
  ...
  for (const u of usersData) {
    const user = await prisma.user.create({ data: u as any });
  ...
  for (let i = 0; i < listingsData.length; i++) {
    ...
    let statut: any = 'APPROUVEE';
    const r = Math.random();
    if (r < 0.2) statut = 'EN_ATTENTE';
    else if (r < 0.3) statut = 'VENDUE';
